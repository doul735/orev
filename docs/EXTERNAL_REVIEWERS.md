...
For a no-HEAD repository, omit `--base <base-sha>` and use a title such as `PD external review for no-head snapshot ${PATCH_ID}`; preserve the patch artifact generated by the fallback path as the immutable reviewed snapshot.

...
- Run this gate only after SUX fixes, executable proof, build, applicable E2E or equivalent coverage, and architecture follow-ups have completed without requiring more tracked-file changes.
...
- If the independent reviewer reports Cancer or Polyp findings, stop before orev, commit, or PR, fix the findings, rerun executable proof/build/architecture checks as applicable, and rerun the independent reviewer gate against the updated final diff. PD 7 may proceed only when independent reviewer Cancer and Polyp counts are 0.
...
7. 아키텍처: [영향 없음|영향 보고됨]
8. Independent reviewer gate: [reviewer/runtime, invocation evidence, immutable diff scope, result]

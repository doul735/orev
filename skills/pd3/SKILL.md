...
- Polyp 발견 → 현재 패스에서 수정하고 `/code-review`를 다시 실행해 Polyp 0건을 증명한다. Polyp 0건이 될 때까지 타입 체크/orev/커밋/PR 단계로 진행하지 않는다.
- Cigarette 발견은 현재 패스에서 수정하고, tracked-file change가 발생하면 `/code-review`를 다시 실행해 새 Cancer/Polyp 0건을 증명한다. 보고서에 cycle evidence, counts, cleanup attempt, remaining items, zero Cancer / Polyp confirmation을 남긴다.
...
- [ ] Polyp 또는 tracked-file Cigarette 수정 발생 시 `/code-review` 재실행으로 Cancer/Polyp 0건 확인
- [ ] tsc 통과
- [ ] orev 결정론적 gate 완료
...
2. Code Review: Cancer 0건, Polyp N건, Cigarette N건 → Polyp/Cigarette 현재 패스 수정, 재리뷰로 Polyp 0건 확인
